...
export default function initAllModels(dbConfig) {
    const { database, username, password, dialect, host, port, socketPath, socketConnectionName } = dbConfig;

    const socketParams = socketConnectionName
        ? {
            socketPath : `${socketPath}${socketConnectionName}`
        }
        : {};
